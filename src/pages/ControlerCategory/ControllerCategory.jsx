...
const ControllerCategory = () => {
    ...
    const schema = yup.object().shape({
        idUsuario: yup.number().required('El ID de usuario es requerido'),
    ...
    const onSubmit = async (data) => {
        try {
            await addCategory(data.idUsuario, data.nombre, data.idEstado, data.fechaCreacion);
        ...
        <Grid container spacing={2} className='control-row' justifyContent="center" alignItems="flex-start" sx={{ minHeight: '100vh', padding: 3, '--Grid-columns': 'unset' }}>
            <div className="control-wrapper">
                <Typography variant="h6" component="div">
                    <h3 style={{fontWeight: 'bold'}}>Agregar Categoría</h3>
                </Typography>
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className='control-row'>
                        <FormLabel className='label'>ID Usuario</FormLabel>
                        <Input variant="outlined" className="input-box" {...register('idUsuario')} />
                        {errors.idUsuario && <p className='msg-error'>{errors.idUsuario.message}</p>}
                    </div>
                    <div className='control-row'>
                    ...
                    <div className='control-row'>
                        <FormLabel className='label'>Fecha de Creación</FormLabel>
                        <Input type="date" variant="outlined" className="input-box" {...register('fechaCreacion')} />
                        {errors.fechaCreacion && <p className='msg-error'>{errors.fechaCreacion.message}</p>}
                    </div>
